...
const MOCK_OWNER_API_RESPONSES = {
  agents: {
    agents: [
      { agent_id: "a-1", name: "搜索小兵", capabilities: ["search", "web"], status: "active", registered_at: "2026-07-01T00:00:00Z" },
      { agent_id: "a-2", name: "代码工兵", capabilities: ["code", "review"], status: "idle", registered_at: "2026-07-01T00:00:00Z" },
    ],
  },
  queue: {
    pending: [
      { task_id: "t-001", capability: "search", status: "pending", description: "搜索竞品最新动态", priority: "high" },
    ],
    active: [
      { task_id: "t-002", capability: "code", status: "running", description: "重构用户模块接口", priority: "critical" },
    ],
  },
  board: {
    entries: [
      { key: "status.search-1", value: "running", updated_at: "2026-07-01T00:00:00Z", updated_by: "agent.search-1" },
    ],
  },
  approvals: {
    approvals: [
      { task_id: "t-001", reason: "搜索结果包含外部数据源，需确认合规", requested_by: "agent.search-1", status: "pending", created_at: "2026-07-01T00:00:00Z" },
    ],
  },
};
...
test("owner dashboard shows all four tabs with data", async ({ page }) => {
  ...
  await page.route("**/api/owner/agents", async (route) => {
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(MOCK_OWNER_API_RESPONSES.agents) });
  });
  await page.route("**/api/owner/queue", async (route) => {
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(MOCK_OWNER_API_RESPONSES.queue) });
  });
  await page.route("**/api/owner/board", async (route) => {
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(MOCK_OWNER_API_RESPONSES.board) });
  });
  await page.route("**/api/owner/approvals", async (route) => {
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(MOCK_OWNER_API_RESPONSES.approvals) });
  });

  // Mock approve endpoint
  await page.route("**/api/owner/approvals/*/approve", async (route) => {
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ approval: { task_id: "t-001", status: "approved" } }) });
  });
  // Mock reject endpoint
  await page.route("**/api/owner/approvals/*/reject", async (route) => {
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ approval: { task_id: "t-001", status: "rejected" } }) });
  });
  ...
  await expect(page.getByRole("cell", { name: "status.search-1" })).toBeVisible();
...
test("approve button calls API and refreshes list", async ({ page }) => {
  ...
  for (const [path, data] of Object.entries({ "/api/owner/agents": MOCK_OWNER_API_RESPONSES.agents, "/api/owner/queue": MOCK_OWNER_API_RESPONSES.queue, "/api/owner/board": MOCK_OWNER_API_RESPONSES.board })) {
  ...
  await expect(page.getByText("Approved").or(page.getByText("No approvals"))).toBeVisible();
});
...
test("reject button calls API and refreshes list", async ({ page }) => {
  ...
  await expect(page.getByText("Rejected").or(page.getByText("No approvals"))).toBeVisible();
});
